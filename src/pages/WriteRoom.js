...
const WriteRoom = () => {
    ...
    //input 타입이 file인 input이 onChange 되었을 때 호출되는 함수
    ...
    const addRoom = () => {
        ...
        //[{},{},{}] ---> [{},{}] ---> ["내용1","내용2"] ---> "내용1*내용2"
        const amenitysText = amenitys.filter(am=>am.checked).map(am=>am.text).join('*');
        console.log("aaa");
        const formData2 = {
            ...formData,
            r_amenity: amenitysText
        }
        // console.log(formData);
            ...
            <form onSubmit={onSubmit}>
                <table className='defaulttable'>
                    <tbody>
                        ...
                        <tr>
                            <td>어메니티</td>
                            <td>{amenitys.map(amenity=><div key={amenity.id}>{amenity.text}
                            <input type="checkbox" value={amenity.text} checked={amenity.checked} onChange={()=>checkClick(amenity.id)} />
                            </div>)}</td>
                        </tr>
                        <tr>
                            <td>이용요금</td>
